...
runnersLargeSizeShirt = runners.filter(item=> (item.shirt_size!='XS')&&(item.shirt_size!='S')&&(item.shirt_size!='M'));
...
let biggestDonation = runners.reduce((max, item)=>{if(item.donation>max){
  return item.donation;
}else{
  return max;
}},0)
console.log("The largest donation is "+biggestDonation);
// Problem 2
//How many people have S size shirt
let smallShirts = runners.filter(item=>item.shirt_size=='S');
console.log("This many people have small shirt size: "+smallShirts.length);
// Problem 3
//List email company names
let emailSites = runners.map(item=>item.email.split('@')[1]);
console.log("This is a list of email providers:")
console.log(emailSites);